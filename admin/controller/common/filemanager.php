		...
		if (isset($this->request->post['directory'])) {
			...
			if ($directories) {
				...
				foreach ($directories as $directory) {
					$dirname = utf8_substr($directory, strlen(DIR_IMAGE . 'data/'));
					if (empty($dirname)) continue;

					//$json[$i]['data'] = basename($directory);
					$json[$i]['data'] = trim(utf8_substr($dirname, intval(mb_strrpos($dirname,'/',0,"UTF-8"))),'/');
					$json[$i]['attributes']['directory'] = $dirname;
	...
	public function upload() {
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {
				...
				if ($this->request->files['image']['size'] > 800000) {
