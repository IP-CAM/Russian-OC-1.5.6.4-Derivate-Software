	...
	public function rewrite($link, $code = '') {
		...
		foreach ($data as $key => $value) {
			switch ($key) {
				...
				case 'path':
				    $postfix = 1; /*можно выставить постфикс, например с окончанием .html для категорий*/
		...
		if (isset($postfix)) {
			...
			$change_url = explode('/',$seo_url);
			if($change_url[3] == 'about' || $change_url[3] == 'services' || $change_url[3] == 'contact'){
				$seo_url .= '.html'; 
			}else{
			    $seo_url .= '/'; 
			}
