			...
			foreach ($results as $result) {
				...
				if ((float)$result['special']) {
					...
					if ($result['currency_id'] != 1) {
                        ...
                        $special_rub = $this->getSymbolPrice($this->currency->format($this->tax->calculate($special, $result['tax_class_id'], $this->config->get('config_tax'))));
                    } else {
                        $special_rub = $this->getSymbolPrice($this->currency->format($this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax'))));
                    }
                    if ($result['currency_id'] != 2) {
                        $currency = $this->model_localisation_currency->getCurrency($result['currency_id']);
                        $special = $this->currency->convert($result['special'], $currency['code'], 'USD');
                        $special = $this->currency->format($this->tax->calculate($special, $result['tax_class_id'], $this->config->get('config_tax')));
						$special_usd =  $this->getSymbolPrice($special);
                    } else {
				...
				$this->data['products'][] = array(
					...
					'square_meter' => $this->model_catalog_category->SelectSquareMeter($result['product_id']),

					'description'  => $descr_plaintext,

					'price'        => $this->getSymbolPrice($price),

                    'rub'          => $this->getSymbolPrice($rub), 
